...
Even though a binary tree can include nodes having degree 2, ano unrooted binary tree is defined more specifically: all internal nodes have degree 3. In turn, a rooted binary tree is such that only the root has degree 2 (all other internal nodes have degree 3). 
...
def inod(n):
    ...
    number_of_edges = 2*n - 3
    number_of_nodes = number_of_edges + 1
    return number_of_nodes - n # THIS IS JUST n-2!!!!


def main():
    return print(inod(test_num))
